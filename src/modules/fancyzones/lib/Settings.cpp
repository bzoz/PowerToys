...
private:
    ...
    const std::wstring m_whitelistAppsName = L"fancyzones_whitelist_apps";
...
IFACEMETHODIMP_(bool) FancyZonesSettings::GetConfig(_Out_ PWSTR buffer, _Out_ int *buffer_size) noexcept
{
    ...
    settings.add_multiline_string(m_whitelistAppsName, IDS_SETTING_WHITELIST_APPS_DESCRIPTION, m_settings.whitelistApps);
...
void FancyZonesSettings::LoadSettings(PCWSTR config, bool fromFile) noexcept try
{
    ...
    if (auto val = values.get_string_value(m_whitelistAppsName))
    {
        m_settings.whitelistApps = std::move(*val);
        m_settings.whitelistAppsArray = splitToArray(m_settings.whitelistApps);
    }
...
void FancyZonesSettings::SaveSettings() noexcept try
{
    ...
    values.add_property(m_whitelistAppsName, m_settings.whitelistApps);
